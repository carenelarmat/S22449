...
!velocity x,y,z, and div
    if (CUSTOM_REAL == SIZE_REAL) then 
      seismograms_der(1:3,irec_local,it) = sngl((nu(:,1,irec)*vxd + nu(:,2,irec)*vyd + nu(:,3,irec)*vzd))
      seismograms_der(4,irec_local,it) = sngl(div)
    else
...
!      call write_seismograms_to_file(seismograms_a,3)
      if (DERIVATIVES_SEISMOS) write(*,*) "Writing derivatives seismos"
      if (DERIVATIVES_SEISMOS) call write_seismograms_to_file_4(seismograms_der)
    else
...
!=====================================================================
  ...
  ! ASCII output format
  call write_output_ASCII_4(one_seismogram, &
            NSTEP,it,SIMULATION_TYPE,DT,t0,myrank, &
            irecord,sisname,final_LOCAL_PATH)


  end subroutine write_one_seismogram_4
  ...
  real(kind=CUSTOM_REAL), dimension(NDIM+1,nrec_local,NSTEP) :: seismograms_der
  ...
  if( .not. WRITE_SEISMOGRAMS_BY_MASTER ) then
    ...
    do irec_local = 1,nrec_local
      ...
      call write_one_seismogram_4(one_seismogram,irec, &
              station_name,network_name,nrec, &
              DT,t0,it,NSTEP,SIMULATION_TYPE, &
              myrank,irecord,component,LOCAL_PATH)
...
! collects the data from all other processes
  ...
  end subroutine write_seismograms_to_file_4
    ...
    irecord = 1
      ...
      do isample = 1,min(it,NSTEP)
        if(irecord == 1) then
          ! distinguish between single and double precision for reals
          if(CUSTOM_REAL == SIZE_REAL) then
            write(IOUT,*) sngl(dble(isample-1)*DT - t0),' ',(seismograms(idim,irec_local,isample),idim = 1,NDIM+1)
          else
            write(IOUT,*) dble(isample-1)*DT - t0,' ',(seismograms(idim,irec_local,isample),idim=1,NDIM+1)
          endif
        else
          call exit_MPI(myrank,'incorrect record label')
        endif
