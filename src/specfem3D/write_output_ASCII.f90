...
! Modified Carene Larmat DERIVATIVE SEIMOGRAMM add subroutine
! write_output_ASCII_4
!
!=====================================================================
...
!Carene write 4 components instead of 3 for DERIVATIVE_SEISMOGRAMS

  subroutine write_output_ASCII_4(one_seismogram, &
              NSTEP,it,SIMULATION_TYPE,DT,t0,myrank, &
...
! the results with the source time function
  ...
  do isample = 1,min(it,NSTEP)
    if(irecord == 1) then
      ...
      if(SEISMOGRAMS_BINARY) then
         ! binary format case
         !tr(isample)=seismograms(iorientation,irec_local,isample)
         tr(isample) = one_seismogram(4,isample)
      else
         ! ASCII format case
         !write(IOUT,*) time_t,' ',seismograms(iorientation,irec_local,isample)
         write(IOUT,*) time_t,' ',one_seismogram(1,isample), one_seismogram(2,isample), one_seismogram(3,isample), & 
             one_seismogram(4,isample)
      endif
  ...
  end subroutine write_output_ASCII_4
