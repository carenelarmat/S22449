...
!Debug
    !write(103,*) myrank,minval(xstore),maxval(xstore),minval(ystore),maxval(ystore),minval(zstore),maxval(zstore)
    !write(103,*) irec,xloc,yloc,elevation(irec)

!    fname=''
!    write(fname,'(i4.4)') myrank
!    fname='free_'//trim(fname)//'.out'
!    open(102,file=fname,status='replace')
...
!
!!          write(102,*) xstore(iglob),',',ystore(iglob),',',zstore(iglob)
!        enddo
!      enddo
!    enddo
!    close(102)
...
! Debug
!    print*,'Z TARGET REIC ',irec,' ',z_target(irec)
